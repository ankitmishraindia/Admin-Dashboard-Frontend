...
function ArticleCard({image,status,tags,type,heading}){
    ...
    return(
        <div className="p-4 bg-white rounded-lg space-y-1">
             <Image
                        src={`/${image}`}
                        alt="articleimage"
             ...
             <div className="flex items-center justify-between">
                    <h2 className="text-[#22285E] text-[20px] font-semibold">{heading}</h2>
                    {status==='Created'?
            ...
            <div className="flex gap-2">
                {tags.map((item)=><div className="px-3 py-2 text-sm bg-gray-100 rounded-md w-fit">{item}</div>)}
            </div>
            <div className="flex justify-between">
                <Link href="/ViewClick">
                   <button className="bg-[var(--violet-light)] text-[var(--violet-text)]  py-2 w-[350px] font-bold rounded-md">View</button>
                </Link>
